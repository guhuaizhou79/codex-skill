...
- [Sequential art and comics](#sequential-art-and-comics)
...
- Decide whether the task is a character sheet, a single panel, a page/strip, or a knowledge comic adapted from source material.
- For recurring characters, lock a canonical character block before generating pages.
- Generate a character reference sheet first when the same character appears in more than one output, then reuse that image as a reference on later generations when possible.
- Specify panel count, reading order, and the core beat of each panel.
- Repeat the same identity anchors (face shape, hair, outfit, silhouette, palette, signature prop) in every page prompt.
- If exact dialogue matters, prefer blank speech bubbles or minimal text and add final lettering later. Otherwise, keep in-image text short and explicit.
...
- comic-character-sheet: Use a neutral background, stable proportions, and front/3/4/profile or expression views; lock palette, costume, and signature props.
- comic-panel: Define the single beat, shot type, emotion, and whether it is a splash, reaction, cutaway, or cover-style image.
- comic-page: Define panel count, reading order, gutters or panel borders, and recurring character anchors; keep each panel beat concrete.
- knowledge-comic: Start from the core message of the page, use one clear visual metaphor per concept, and distinguish narration from dialogue.
...
For copy/paste prompt specs (examples only), see `references/sample-prompts.md`. For comic and recurring-character workflow, see `references/comic-workflow.md`. This file focuses on principles, specificity, and iteration patterns.
