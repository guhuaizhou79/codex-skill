---
name: "imagegen"
description: "Generate or edit raster images when the task benefits from AI-created bitmap visuals such as photos, illustrations, comics, manga/webtoon pages, character sheets, textures, sprites, mockups, or transparent-background cutouts. Use when Codex should create a brand-new image, transform an existing image, derive visual variants from references, or produce sequential-art assets with recurring-character consistency, and the output should be a bitmap asset rather than repo-native code or vector. Do not use when the task is better handled by editing existing SVG/vector/code-native assets, extending an existing icon or logo system, or building the visual directly in HTML/CSS/canvas."
---

# Image Generation Skill

Generate or edit images for the current project (for example website assets, game assets, UI mockups, product mockups, wireframes, comics, character sheets, photorealistic images, or infographics).
...
Shared prompt guidance for both modes lives in `references/prompting.md`, `references/sample-prompts.md`, and `references/comic-workflow.md`.
...
- Generate a comic panel, comic page, webtoon slice, cover, or character reference sheet
- Build a short comic workflow from notes or source material into page briefs, character sheets, and final page images
...
2. **Scope:** is this a standalone image, a recurring-character asset set, or sequential art?
3. **Execution strategy:** is this one asset or many assets/variants?
...
Scope:
- If the same character or subject must recur across multiple outputs, treat it as **recurring-character work**.
- If the user wants a multi-panel strip/page, webtoon slice, or article-to-comic adaptation, treat it as **sequential-art work**.
- For recurring-character or sequential-art work, load `references/comic-workflow.md` before writing prompts.
...
- Load `references/comic-workflow.md` for comic panels, comic pages, strips, webtoons, knowledge comics, or any recurring-character image set.
- Generate a character reference sheet first when the same character must remain consistent across multiple outputs.
- Reuse the character sheet as a reference image on later generations when possible; otherwise repeat the same character anchor block in every page prompt.
- Decide before rendering whether captions and speech bubbles should be blank, verbatim, or omitted. If exact dialogue matters, prefer blank balloons or minimal text and add final lettering later.
...
- `illustration-story`: story illustrations, children's-book art, and narrative scenes.
- `comic-character-sheet`: turnarounds, expression sheets, cast lineups, recurring-character anchors.
- `comic-panel`: single-panel story beats, splash panels, covers, poster-like comic scenes.
- `comic-page`: comic strips, manga pages, webtoon slices, and multi-panel sequential art.
- `knowledge-comic`: educational, tutorial, biography, and explainer comics adapted from source material.
...
Character anchors: <canonical recurring-character details> (optional)
Panel plan: <panel count, reading order, beat summary> (optional)
...
Notes:
- `Asset type`, `Input images`, `Character anchors`, and `Panel plan` are prompt scaffolding, not dedicated CLI flags.
...
- Include intended use (ad, UI mock, infographic, comic page, character sheet) to set the mode and polish level.
...
- For recurring-character or sequential-art work, keep the same character anchors stable across every follow-up prompt.
...
Comic planning and character-consistency workflow: `references/comic-workflow.md`.

## Guidance by asset type
Asset-type templates (website assets, game assets, wireframes, logo, comics, and character sheets) are consolidated in `references/sample-prompts.md`.
...
- `references/comic-workflow.md`: shared workflow for comics, character sheets, page briefs, and recurring-character consistency.
