...
- 启动：`node "C:\Users\凯丰达\.codex\skills\web-access\scripts\cdp-proxy.mjs"`
- 启动后持续运行，不建议主动停止（重启需 Chrome 重新授权）
- 强制停止：Windows 下用任务管理器，或执行 `Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like '*cdp-proxy.mjs*' } | Remove-CimInstance`
...
```bash
curl.exe -s -X POST "http://localhost:3456/setFiles?target=ID" -d '{"selector":"input[type=file]","files":["C:\\path\\to\\file1.png","C:\\path\\to\\file2.png"]}'
```
...
```bash
curl.exe -s "http://localhost:3456/screenshot?target=ID&file=$env:TEMP\\shot.png"
```
