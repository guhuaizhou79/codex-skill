...
> Codex/OpenCode 适配版：当前默认安装目录为 `C:\Users\凯丰达\.codex\skills\web-access`。在 Windows PowerShell 中执行 HTTP 示例时，优先使用 `curl.exe`，不要依赖 PowerShell 的 `curl` 别名。
...
```bash
node "C:\Users\凯丰达\.codex\skills\web-access\scripts\check-deps.mjs"
```

若运行环境不是当前机器，默认把 skill 根目录视为 `${CODEX_HOME:-$HOME/.codex}/skills/web-access`，不要再使用 `CLAUDE_SKILL_DIR`。
...
| 场景 | 工具 |
|------|------|
| 搜索摘要或关键词结果，发现信息来源 | **优先当前 Agent 的内置联网/搜索工具**；当前环境若无独立搜索工具，则用 `webfetch` 读取搜索结果页、导航页或官网索引页 |
| URL 已知，需要从页面定向提取特定信息 | **webfetch**（拉取网页内容并定向提取） |
| URL 已知，需要原始 HTML 源码（meta、JSON-LD 等结构化字段） | **bash + `curl.exe`** |
| 非公开内容，或已知静态层无效的平台（小红书、微信公众号等公开内容也被反爬限制） | **浏览器 CDP**（直接，跳过静态层） |
| 需要登录态、交互操作，或需要像人一样在浏览器内自由导航探索 | **浏览器 CDP** |

浏览器 CDP 不要求 URL 已知——可从任意入口出发，通过页面内搜索、点击、跳转等方式找到目标内容。内置静态联网工具、`webfetch`、`curl.exe` 均不处理登录态。

**Jina**（可选预处理层，可与 `webfetch`/`curl.exe` 组合使用，由于其特性可节省 token 消耗，请积极在任务合适时组合使用）：第三方网络服务，可将网页转为 Markdown，大幅节省 token 但可能有信息损耗。调用方式为 `https://r.jina.ai/http://example.com` 或 `https://r.jina.ai/http://...` 对应目标 URL，适合文章、博客、文档、PDF 等以正文为核心的页面；对数据面板、商品页等非文章结构页面可能提取到错误区块。
...
所有操作通过 HTTP API 调用。当前机器在 PowerShell 中请使用 `curl.exe`：
...
curl.exe -s -X POST "http://localhost:3456/setFiles?target=ID" -d '{"selector":"input[type=file]","files":["C:\\path\\to\\file.png"]}'
...
任务包含多个**独立**调研目标时（如同时调研 N 个项目、N 个来源），鼓励合理分治给子 Agent 并行执行，而非主 Agent 串行处理。在 Codex/OpenCode 中优先使用 `task` 工具启动子 Agent。
...
- 在 Codex/OpenCode 的子 Agent prompt 中写 `必须加载 web-access skill 并遵循指引`。无需把 skill 全文复制进 prompt。
- 子 Agent 有自主判断能力。主 Agent 的职责是说清楚**要什么**，仅在必要与确信时限定**怎么做**。过度指定步骤会剥夺子 Agent 的判断空间，反而引入主 Agent 的假设错误。**避免 prompt 用词对子 Agent 行为的暗示**：「搜索 xx」会把子 Agent 锚定到静态搜索路径，而实际上有些反爬站点需要 CDP 直接访问主站才能有效获取内容。主 Agent 写 prompt 时应描述目标（「获取」「调研」「了解」），避免用暗示具体手段的动词（「搜索」「抓取」「爬取」）。
...
| 需要 CDP 浏览器或长时间运行的任务 | 几次 `webfetch` / Jina 就能完成的轻量查询 |
